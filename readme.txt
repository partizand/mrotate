...
KeepPeriod=180 
; Какую дату брать у файлов (по умолчанию Created), сверяется только первая буква
DateMode=Modified
; Дата на замену в параметрах %d, по умолчанию Now - текущая, modify (проверяется только первая буква)
DateReplace=Created
; Скрипт перед ротацией
prerotate=d:\load.bat
; Скрипт после ротации
postrotate=d:\unload.bat
; Скрипты будут выполнятся для каждого файла в отдельности (по умолчанию false)
;sharedscripts=yes
